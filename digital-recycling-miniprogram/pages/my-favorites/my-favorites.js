...
Page({
  ...
  loadData() {
    userApi.getFavorites({ page: this.data.page, pageSize: PAGE_SIZE }).then(res => {
  ...
  onItemTap(e) { const item = this.data.list[e.currentTarget.dataset.index]; if (item) wx.navigateTo({ url: `/pages/price-quote/price-quote?productId=${item.product_id || item.id}` }) }
})
